import { allBannerList } from "../banners";
import { Collection } from "discord.js";
import { Race, Type, Grade, Event, Affection, Unit } from "../../models/unit";
import { ALL_RACES, ALL_TYPES, ALL_GRADES, ALL_EVENT, ALL_AFFECTIONS, unitList } from "../units";
...
export function getRandomUnit(races: Race[], types: Type[], grades: Grade[], events: Event[], affections: Affection[], names: string[], banners: string[], excepting: Unit[]) {
	let possibleUnits = getUnitsMatching(races, types, grades, events, affections, names, banners, excepting);

	if (possibleUnits.size === 0) return null;
...
export function replaceDuplicates(
	...
	for (let unit = 0; unit < teamToCheck.length; unit++) {
		for (let i = 0; i < 500; i++) {
			if (checkNames(unit)) break;

			if (teamSimpleNames[i] === "") throw new RangeError("Not enough Units available");
		}
	}
