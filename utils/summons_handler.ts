import { DefaultEmbed, sendNavigationMenu } from "./embeds";
import { MessageActionRow, MessageButton, MessageAttachment, CommandInteraction, GuildMember, Collection } from 'discord.js';
import { bannerMultiDisplay, bannerWhaleDisplay, bannerRotationDisplay } from "./display";
import { Grade, Unit } from './units';
...
export async function single(
    ...
    ref: boolean = true) {
        ...
        for(let i = 1; i <= amount; i++)
            units.push(banner.unitByChance())
        ...
        await sendNavigationMenu(
            ...
            interaction,
            {
                ...
                embeds: [new DefaultEmbed()
                    .setTitle(banner.prettyName + " (1x summon)")
            ...
            ref,
            {
                ...
                update: async (i, _, c, p) => {
                    ...
                    await i.editReply({
                        ...
                        embeds: [new DefaultEmbed()
                            .setTitle(`${banner.prettyName} (1x summon) [${p + 1}.]`)
                            .setImage("attachment://unit.png")
                        ],
                    })
                }
    ...
    ref: boolean = true) {
        if(rotation) {
            ...
            for(let i = 0; i < (banner.loyalty / 30) * 11; i++) {
                ...
                if(units.has(unit))
                    units.set(unit, units.get(unit) + 1)
                else
                    units.set(unit, 1)
            }
            ...
            if(units.size > 0)
                units = new Collection(Array.from(units).sort((a, b) => b[0].grade.toInt() - a[0].grade.toInt()))

            return await interaction.editReply({
                files: [new MessageAttachment(await bannerRotationDisplay(units), "units.png")],
                content: (person === interaction.member ? " " : `Rotation for ${person}`),
                embeds: [new DefaultEmbed()
                    .setTitle(`1 Rotation on ${banner.prettyName} (${banner.loyalty} Gems)`)
        ...
        for(let i = 1; i <= amount; i++) {
            ...
            for(let unit = 0; unit < banner.bannerType; unit++)
                multi.push(banner.unitByChance())
            units.push(multi)
        }
        ...
        await sendNavigationMenu(
            ...
            interaction,
            {
                ...
                files: [new MessageAttachment(await bannerMultiDisplay(units[0], banner.bannerType === 5), "units.png")],
                embeds: [new DefaultEmbed()
                    .setTitle(`${banner.prettyName} (${banner.bannerType === 11 ? "11" : "5"}x summon)`)
                    .setImage("attachment://units.png")
                ]
            },
            ref,
            {
                ...
                update: async (i, _, c, p) => {
                    ...
                    await i.deferUpdate()
                    await i.editReply({
                        files: [new MessageAttachment(await bannerMultiDisplay(c, banner.bannerType === 5), "units.png")],
                        embeds: [new DefaultEmbed()
                            .setTitle(`${banner.prettyName} (${banner.bannerType === 11 ? "11" : "5"}x summon) [${p + 1}.]`)
                            .setImage("attachment://units.png")
                        ],
                    })
                }
...
export async function whale(
    ...
    ref: boolean = true) {
        ...
        while(!containsAnyUnit(drawnUnits)[0]) {
            ...
            tempDrawn.filter(u => u.grade === Grade.SSR).forEach(u => {
                if(drawnSSRs.has(u))
                    drawnSSRs.set(u, drawnSSRs.get(u) + 1)
                else
                    drawnSSRs.set(u, 1)
            })
            tempDrawn.length = 0
        ...
        await interaction.editReply({
            files: [new MessageAttachment(await bannerWhaleDisplay(drawnUnits, drawnSSRs, banner.bannerType === 5), "units.png")],
            embeds: [new DefaultEmbed()
                .setTitle(`${banner.prettyName} (Whaled summons [${counter}x])`)
                .setImage("attachment://units.png")
            ],
            components: [],
            content: null
...
export async function infos(interaction: CommandInteraction, banner: Banner, ref: boolean = true) {
    await sendNavigationMenu(
        ...
        interaction,
        {
            ...
            embeds: [new DefaultEmbed()
                .setTitle(`Units in ${banner.prettyName}  [Page: 1]`)
                .setImage("attachment://units.png")
            ]
        },
        ref,
        {
            ...
            update: async (i, m, c, p) => {
                ...
                await i.update({
                    embeds: [new DefaultEmbed()
                        .setTitle(`Units in ${banner.prettyName} [Page: ${p + 1}]`)
                        .setImage("attachment://units.png")
                    ],
