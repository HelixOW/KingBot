import { MessageEmbed, CommandInteraction, MessageActionRow, MessageButton, MessageInteraction, Message, InteractionCollector, MessageComponentInteraction, WebhookEditMessageOptions, MessageAttachment } from "discord.js"
    ...
    options: {
        ...
        for(const option of options) {
            ...
            collector.on('end', async (collected, reason) => {
                if(option.postCollect)
                    await option.postCollect(reason, interaction)
            })
        }
}
...
export async function sendNavigationMenu<T>(
    ...
    options: {
        ...
        await sendMenu(
            ...
            {
                embeds: reply.embeds,
                components: check ? [new MessageActionRow().addComponents(
                    new MessageButton()
                        .setCustomId("prev")
                        .setStyle("PRIMARY")
                        .setEmoji(previous),
